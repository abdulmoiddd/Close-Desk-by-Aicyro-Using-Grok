...
export default function AicyroChatbot() {
  ...
  const [botConfig, setBotConfig] = useState({
    ...
    customAvatarSvg: "",
    botVoice: "Joanna",
    voiceEnabled: true, // Default to true, controlled by Firebase
  });
  ...
  const handleStartVoiceCall = async () => {
    ...
    try {
      ...
      if (
        ...
        triggerNotification(
          "Microphone Blocked",
          "Please allow microphone access or continue in text mode.",
        );
      } else if (
        ...
        triggerNotification(
          "Hardware Error",
          "No working microphone detected. Falling back to text mode.",
        );
      } else {
        triggerNotification(
          "Connection Error",
          "Voice calls require a secure connection. Falling back to text.",
        );
      }
      return;
    }
  ...
  const handleEndVoiceCall = () => {
    try {
      ...
      setVoiceState("IDLE");
      activeResponseRef.current = false;
      triggerNotification("Call Ended", "Voice connection disconnected.");
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          setBotConfig({
            ...
            voiceEnabled: data.voiceEnabled !== false, // Default true unless explicitly turned off
          });
        }
      } catch (err) {
                ...
                <div className="flex-1 flex flex-col items-center justify-center bg-[var(--background)] p-6 min-h-full pb-[calc(env(safe-area-inset-bottom,0px)+24px)]">
                  ...
                  >
                    ...
                    {voiceState === "LISTENING" && (
                      <div className="absolute inset-0 rounded-full border-4 border-green-500 animate-pulse opacity-50"></div>
                    )}
                    {voiceState === "PROCESSING" && (
                      <div className="absolute inset-0 rounded-full border-4 border-[var(--accent-blue)] border-t-transparent animate-spin opacity-70"></div>
                    )}
                    {voiceState === "SPEAKING" && (
                      <>
                        <div className="absolute inset-0 rounded-full border-4 border-[var(--primary)] animate-ping opacity-30"></div>
                        <div
                          className="absolute inset-[-20px] rounded-full border-2 border-[var(--primary)] animate-ping opacity-20"
