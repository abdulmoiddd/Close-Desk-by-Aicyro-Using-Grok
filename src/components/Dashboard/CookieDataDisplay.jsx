...
const CustomMapMarker = ({ cookie, zoom, now, isActive, onClick }) => {
  ...
  // Only fetch the exact location string when the pin is clicked to save API requests
  useEffect(() => {
    if (!isActive) return;
    if (placeName) return; // Already fetched
  ...
  // Inverse scale to keep the pin the same visual size
  const markerScale = 1 / zoom;

  // Custom scale for the text so it actively zooms in (gets larger) as the user zooms the map
  ...
  return (
    <Marker coordinates={[cookie.location.lng, cookie.location.lat]}>
      ...
      >
        {/* Ping Animation for newly active users */}
        {isJustNow && (
          <circle
            r={14}
            fill="var(--primary)"
      ...
      {/* Popup Text (Visible only when active) */}
      {isActive && (
        <g
...
// Helper Component: Fetches City/Country name and renders the clickable link in the Cards
...
export default function CookieDataDisplay() {
  ...
  // Filter cookies based on view mode
  ...
  const handleArchiveSelected = async () => {
    try {
      const archivePromises = selectedCookies.map((id) =>
        update(ref(db, `user_cookies/${id}`), { isArchived: true }),
      );
  ...
  const handleRestoreSelected = async () => {
    try {
      const restorePromises = selectedCookies.map((id) =>
        update(ref(db, `user_cookies/${id}`), { isArchived: false }),
      );
  ...
  const handleDeleteSelected = async () => {
    if (
      !window.confirm(
        `Are you sure you want to completely delete ${selectedCookies.length} session(s)?`,
      )
    )
      return;
    ...
    <section className="w-full max-w-7xl mx-auto p-6 fade-in">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            {archivedCookies.length > 0 && (
              <button
                ...
                className="ml-3 px-3 py-1.5 rounded-full text-xs font-bold tracking-wider uppercase border transition-all
                  bg-[var(--background)] border-[var(--border-color)] text-[var(--foreground-muted)] hover:text-[var(--foreground)] hover:border-[var(--foreground-muted)]"
              >
                {viewMode === "active"
        ...
        {selectedCookies.length > 0 && (
          <div className="flex items-center gap-2 sm:gap-3 animate-in fade-in zoom-in-95 duration-300 flex-wrap">
            <span className="text-sm font-medium text-[var(--foreground-muted)] mr-2">
              {selectedCookies.length} Selected
            </span>
            <button
              onClick={handleSelectAll}
              className="px-4 py-2 text-sm font-semibold rounded-lg bg-[var(--background)] border border-[var(--border-color)] text-[var(--foreground)] hover:bg-[var(--primary)] hover:text-white hover:border-[var(--primary)] transition-colors shadow-sm"
            >
              {selectedCookies.length === displayCookies.length
                ? "Deselect All"
                : "Select All"}
            ...
            {viewMode === "active" ? (
              <button
                ...
                className="px-4 py-2 text-sm font-semibold rounded-lg bg-indigo-500/10 text-indigo-500 border border-indigo-500/20 hover:bg-indigo-500 hover:text-white transition-colors shadow-sm"
              >
                Archive
              </button>
            ) : (
              <button
                ...
                className="px-4 py-2 text-sm font-semibold rounded-lg bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 hover:bg-emerald-500 hover:text-white transition-colors shadow-sm"
              >
                Restore
              </button>
            )}
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-semibold rounded-lg bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500 hover:text-white transition-colors shadow-sm"
            >
              Delete
            </button>
          </div>
            ...
            return (
              <div
                ...
                className={`theme-glass-card p-6 group relative overflow-hidden flex flex-col justify-between transition-all duration-300 cursor-pointer hover:-translate-y-1 hover:shadow-xl
                  ${
                    isSelected
                      ? "border-[var(--primary)] bg-[var(--primary)]/5 shadow-[0_8px_30px_rgb(0,0,0,0.12)] ring-2 ring-[var(--primary)] ring-offset-2 ring-offset-[var(--background)]"
