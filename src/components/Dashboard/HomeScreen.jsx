...
export default function HomeScreen({ onLogout }) {
  ...
  // --- FIREBASE REALTIME DATABASE LISTENERS ---
  useEffect(() => {
    // 1. Listen for LIVE Data (Updated to point to 'analytics/visitors')
    ...
    const unsubscribeLeads = onValue(
      ...
      (snapshot) => {
        const data = snapshot.val();
        if (data) {
          // Map nested Firebase JSON into flat properties
          const liveData = Object.keys(data)
            .map((key) => {
              ...
              return {
                ...visitor, // Keep raw data available
    ...
    // 2. Listen for Analytics Summary
    const analyticsRef = ref(db, "analytics/summary");
    const unsubscribeAnalytics = onValue(
      analyticsRef,
      (snapshot) => {
  ...
  const filteredVisitors = useMemo(() => {
    ...
    today.setHours(0, 0, 0, 0); // Lock to Midnight to prevent boundary bugs
  ...
  // Use Exact array counts instead of fragile ratio interpolation math
  const scaledVisitors =
  ...
  // Define an actual "Lead" as someone who engaged in convo OR left contact info
  ...
  // Funnel conversion performance
  const leadsCaptured = capturedLeadsArr.length;
  const qualifiedLeads = capturedLeadsArr.filter(
    (l) => l.lead_score === "High" || l.lead_score === "Medium",
  ).length;
  const bookedAppointments = capturedLeadsArr.filter(
    (l) =>
      l.booking_status === "Meeting Booked" || l.booked_slot || l.display_time,
  ).length;
  ...
  const chartData = useMemo(() => {
    ...
    // Setup empty timeline array using robust milliseconds instead of .setDate
    for (let i = effectiveTimeFrame - 1; i >= 0; i--) {
    ...
    // Direct mapping of exact activity values to corresponding chart day
