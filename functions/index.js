// functions/index.js
const functions = require("firebase-functions/v1"); // 🔥 Reverted back to the correct v1 import
...
async function getDynamicTransporter() {
  ...
  // Route via Custom Company SMTP if specified
  if (provider === "custom" && config.smtpHost) {
    const port = parseInt(config.smtpPort) || 465;
    transportConfig = {
      host: config.smtpHost,
      port: port,
      secure: port === 465, // SSL requires secure: true. TLS (587) requires secure: false.
      auth: {
        user: userEmail,
        pass: userPass,
      },
    ...
    // Default fallback to Google Workspace / Gmail routing
    transportConfig = {
      service: "gmail",
      auth: {
        user: userEmail,
